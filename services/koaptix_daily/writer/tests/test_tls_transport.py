"""Offline tests of the real writer: fake credentials, no live connector."""
...
APP_NAMES = ("CRON_SECRET", "S1_WRITER_DSN", "S1_VERIFY_URL", "S1_VERIFY_REQUEST_SECRET")
...
def setUpModule():
    ...
    try:
        ...
        with mock.patch.object(os, "environ", KeysOnlyEnvironment()):
            writer = load_writer("_koaptix_tls_writer_under_test")
        for name in ("json_call", "transaction", "remaining", "phase_observation",
                     "verifier_request", "reconcile", "run_phase",
                     "append_observations", "run_workflow"):
            _suite_stack.enter_context(mock.patch.object(writer, name, side_effect=blocked))
...
class TransportTests(unittest.TestCase):
    def setUp(self):
        ...
        writer._verified_ca_path.cache_clear()
        self.addCleanup(writer._verified_ca_path.cache_clear)
    ...
    def assert_safe_failure(self, operation):
        ...
        with redirect_stdout(output), redirect_stderr(errors):
            with self.assertRaises(writer.WorkflowStop) as caught:
                operation()
        ...
        self.assertEqual(caught.exception.ack, "UNOBSERVED")
        self.assertIsNone(caught.exception.operation_result)
    ...
    def reject_dsn(self, dsn):
        with mock.patch.object(psycopg, "connect") as connector:
            with mock.patch.object(writer, "_verified_ca_path") as ca_check:
                self.assert_safe_failure(lambda: writer.connect(dsn))
        connector.assert_not_called()
        ca_check.assert_not_called()

    def test_canonical_credential_core(self):
        core = writer._credential_core(self.canonical())
    ...
    def test_password_space_roundtrip(self):
        ...
        self.assertEqual(writer._credential_core(self.canonical(values)), values)
    ...
    def test_length_16384_boundary_is_accepted(self):
        ...
        self.assertEqual(len(writer._credential_core(dsn)["password"]), 16384 - overhead)
    ...
    def test_guard_allows_application_names_without_values(self):
        writer._guard_transport_environment()
        self.assertEqual(self.environment.value_reads, 0)
    ...
    def test_environment_guard_precedes_parsing(self):
        ...
        with mock.patch.object(writer, "_credential_core") as parser:
            self.assert_safe_failure(lambda: writer.connect(self.canonical()))
    ...
    def test_exact_trusted_overlay_and_preserved_connection_options(self):
        ...
        with mock.patch.object(psycopg, "connect", return_value=sentinel) as connector:
            self.assertIs(writer.connect(self.canonical()), sentinel)
    ...
    def test_connector_error_is_sanitized_without_logging(self):
        ...
        with mock.patch.object(psycopg, "connect", side_effect=RuntimeError(marker)):
            with mock.patch("logging.Logger._log") as logger:
                self.assert_safe_failure(lambda: writer.connect(self.canonical()))
                try:
                    writer.connect(self.canonical())
                except writer.WorkflowStop as exc:
    ...
    def test_import_is_inert_and_ca_validation_is_lazy(self):
        ...
        with mock.patch.object(Path, "read_bytes", side_effect=blocked):
            with redirect_stdout(output), redirect_stderr(errors):
                fresh = load_writer("_koaptix_tls_writer_inert_import")
    ...
    def test_accepted_public_ca_hash_and_absolute_sibling(self):
        ...
        result = Path(writer._verified_ca_path())
    ...
    def test_ca_missing_metadata_fails_before_connector(self):
        with mock.patch.object(Path, "lstat", side_effect=FileNotFoundError("SYNTHETIC")):
            with mock.patch.object(psycopg, "connect") as connector:
                self.assert_safe_failure(lambda: writer.connect(self.canonical()))
        connector.assert_not_called()
        self.assertEqual(writer._verified_ca_path.cache_info().currsize, 0)

    def test_ca_wrong_bytes_fail(self):
        with mock.patch.object(Path, "read_bytes", return_value=b"SYNTHETIC-WRONG-CA"):
            self.assert_safe_failure(writer._verified_ca_path)
    ...
    def test_ca_wrong_module_sibling_fails(self):
        with mock.patch.object(writer, "__file__", str(TEST_PATH)):
            self.assert_safe_failure(writer._verified_ca_path)
    ...
    def test_ca_cwd_independent(self):
        ...
        try:
            ...
            self.assertEqual(writer._verified_ca_path(), str(CA_PATH.resolve()))
        finally:
            os.chdir(original)
    ...
    def test_successful_ca_verification_is_cached(self):
        ...
        with mock.patch.object(Path, "read_bytes", return_value=data) as reader:
            first = writer._verified_ca_path()
            second = writer._verified_ca_path()
        self.assertEqual(first, second)
        reader.assert_called_once()
        self.assertEqual(writer._verified_ca_path.cache_info().currsize, 1)
        self.assertEqual(writer._verified_ca_path.cache_info().hits, 1)
...
for _name, _factory in {
    ...
    "verifier_user": lambda self: self.canonical({**FAKE_CORE, "user": "koaptix_publication_verifier.dsnqbadkyfmzeikzgvqp"}),
    "management_user": lambda self: self.canonical({**FAKE_CORE, "user": "postgres.dsnqbadkyfmzeikzgvqp"}),
}.items():
