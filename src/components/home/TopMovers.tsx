  ...
  return (
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-slate-700/50 bg-[#0b1118]/80 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.03),0_18px_40px_rgba(0,0,0,0.3)] sm:p-5">
      <div className="flex items-center justify-between border-b border-slate-800/80 pb-2.5">
        <div>
          <p className="text-[10px] uppercase tracking-[0.24em] text-slate-400">주간 움직임 관측</p>
          <h2 className="mt-0.5 text-base font-bold text-slate-100 sm:text-lg">이번 주 순위 변화</h2>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        {isNeutral && (
          <div className="rounded-lg border border-slate-700/50 bg-slate-800/20 p-3 text-center">
            <span className="text-[11px] font-medium text-slate-400">이번 주에는 눈에 띄는 순위 변화가 아직 없습니다.</span>
            <div className="mt-3 flex flex-col gap-1.5 border-t border-slate-700/50 pt-2 text-left">
              <span className="mb-1 text-[9px] uppercase tracking-wider text-slate-500">현재 추정 시총 상위 단지</span>
              {topCaps.map(item => (
        ...
        {hasUpdrafts && (
          ...
          <div className="rounded-lg border border-rose-500/20 bg-rose-500/[0.02] p-2.5">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-[10px] font-bold text-rose-400">📈 주간 순위 상승</span>
              <span className="rounded bg-rose-500/10 px-1.5 py-0.5 text-[8px] uppercase tracking-widest text-rose-400">상승 관측</span>
            </div>
        ...
        {hasDrags && (
          ...
          <div className="rounded-lg border border-blue-500/20 bg-blue-500/[0.02] p-2.5">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-[10px] font-bold text-blue-400">📉 주간 순위 하락</span>
              <span className="rounded bg-blue-500/10 px-1.5 py-0.5 text-[8px] uppercase tracking-widest text-blue-400">하락 관측</span>
            </div>
            <div className="flex flex-col gap-1.5">
              {drags.map(item => (
                <div key={item.complexId} className="flex items-center justify-between text-[11px] sm:text-xs">
